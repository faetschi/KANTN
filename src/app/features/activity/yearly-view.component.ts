    ...
    <div class="space-y-4 stagger">
      @if (plans.length === 0) {
        ...
        <div class="text-center py-8">
          <p class="text-gray-400 text-sm">0 contributions and 0 active days.</p>
        </div>
      }
...
export class YearlyViewComponent {
  ...
  getYearData(planId: string) {
    return this.yearData.find(yd => yd.planId === planId)?.contributions || [];
  }

  getStreak(planId: string): number {
    return this.yearData.find(yd => yd.planId === planId)?.streak || 0;
  }

  getTotalActiveDays(planId: string): number {
    return this.yearData.find(yd => yd.planId === planId)?.totalActiveDays || 0;
  }
